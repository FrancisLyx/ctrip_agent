...
from graph_chat.agent_assistant import (
...
def build_flight_graph(builder: StateGraph) -> StateGraph:
    ...
    builder.add_node(
        "update_flight", CtripAssistant(update_flight_runnable)
    )  # 添加处理航班更新的实际节点
    builder.add_edge(
    ...
    def route_update_flight(state: dict):
        ...
        safe_tool_names = [
            t.name for t in update_flight_safe_tools
        ]  # 获取所有安全工具的名字
        if all(
            tc["name"] in safe_tool_names for tc in tool_calls
        ):  # 如果所有调用的工具都是安全工具
...
def build_car_graph(builder: StateGraph) -> StateGraph:
    ...
    builder.add_node(
        "book_car_rental", CtripAssistant(book_car_rental_runnable)
    )  # 添加处理租车预订的实际节点
    builder.add_edge(
    ...
    def route_book_car_rental(state: dict):
        ...
        if did_cancel:
            return "leave_skill"  # 如果用户请求取消或退出，则跳转至leave_skill节点
        safe_toolnames = [
            t.name for t in book_car_rental_safe_tools
        ]  # 获取所有安全工具的名字
        if all(
            tc["name"] in safe_toolnames for tc in tool_calls
        ):  # 如果所有调用的工具都是安全工具
...
def builder_hotel_graph(builder: StateGraph) -> StateGraph:
    ...
    builder.add_node(
        "book_hotel", CtripAssistant(book_hotel_runnable)
    )  # 添加处理酒店预订的实际节点
    ...
    def route_book_hotel(state: dict):
        ...
        if did_cancel:
            return "leave_skill"  # 如果用户请求取消或退出，则跳转至leave_skill节点
        safe_toolnames = [
            t.name for t in book_hotel_safe_tools
        ]  # 获取所有安全工具的名字
        if all(
            tc["name"] in safe_toolnames for tc in tool_calls
        ):  # 如果所有调用的工具都是安全工具
...
def builder_excursion_graph(builder: StateGraph) -> StateGraph:
    ...
    builder.add_node(
        ...
        create_entry_node(
            "旅行推荐助理", "book_excursion"
        ),  # 创建入口节点，指定助理名称和新对话状态
    )
    builder.add_node(
        "book_excursion", CtripAssistant(book_excursion_runnable)
    )  # 添加处理游览预订的实际节点
    builder.add_edge(
    ...
    def route_book_excursion(state: dict):
        ...
        if did_cancel:
            return "leave_skill"  # 如果用户请求取消或退出，则跳转至leave_skill节点
        safe_toolnames = [
            t.name for t in book_excursion_safe_tools
        ]  # 获取所有安全工具的名字
        if all(
            tc["name"] in safe_toolnames for tc in tool_calls
        ):  # 如果所有调用的工具都是安全工具
